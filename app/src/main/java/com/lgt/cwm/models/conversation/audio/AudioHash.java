...
public final class AudioHash {
    ...
    public AudioHash(@NonNull AudioWaveFormData audioWaveForm) {
        this(Base64.encodeToString(audioWaveForm.getWaveForm(), Base64.DEFAULT), audioWaveForm);
    }
    ...
    public static @Nullable AudioHash parseOrNull(@Nullable String hash) {
        ...
        try {
            //todo: check duration
            return new AudioHash(hash, new AudioWaveFormData(0, Base64.decode(hash, Base64.DEFAULT) ));
        } catch (Exception e) {
            return null;
        }
    }
