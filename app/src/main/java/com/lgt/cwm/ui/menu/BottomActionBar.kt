 ...
 */
class BottomActionBar(context: Context, attributeSet: AttributeSet) : LinearLayout(context, attributeSet) {
    ...
    init {
        ...
        setBackgroundResource(R.drawable.bottom_action_bar_background)
    ...
    private fun present(items: List<ActionItem>) {
        ...
        renderableItems.forEach { item ->
            val view: View = LayoutInflater.from(context).inflate(R.layout.bottom_action_bar_item, this, false)
            addView(view)
            bindItem(view, item)
        }
    ...
    private fun bindItem(view: View, item: ActionItem) {
        val icon: ImageView = view.findViewById(R.id.bottom_action_bar_item_icon)
        val title: TextView = view.findViewById(R.id.bottom_action_bar_item_title)
