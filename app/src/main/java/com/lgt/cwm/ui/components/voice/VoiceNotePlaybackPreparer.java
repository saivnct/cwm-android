...
import com.lgt.cwm.util.concurrent.SimpleTask;
...
final class VoiceNotePlaybackPreparer implements MediaSessionConnector.PlaybackPreparer {
    ...
    private static final String   TAG      = VoiceNotePlaybackPreparer.class.getSimpleName();
    ...
    public void onPrepareFromUri(@NonNull Uri uri, boolean playWhenReady, @Nullable Bundle extras) {
        ...
        SimpleTask.run(EXECUTOR,
                () -> {
                    if (singlePlayback) {
                        if (messageId != -1) {
                            return loadMediaItemsForSinglePlayback(messageId, uri);
                        } else {
                            return loadMediaItemsForDraftPlayback(threadId, uri);
                        }
                    } else {
                        return loadMediaItemsForConsecutivePlayback(messageId, uri);
                    }
                },
                mediaItems -> {
                    Log.d("VoiceNoteMedia", "mediaItems " + mediaItems.get(0).mediaMetadata.title);
    ...
    private void applyDescriptionsToQueue(@NonNull List<MediaItem> mediaItems) {
        Log.d("applyDescriptions", "mediaItems size " + mediaItems.size());
        ...
        for (MediaItem mediaItem : mediaItems) {
            ...
            Log.d("applyDescriptions", "holderIndex " + holderIndex + " currentIndex " + currentIndex);
            ...
            if (holderIndex != -1) {
                ...
                Log.d("applyDescriptions", "aadd");

                player.addMediaItem(insertLocation, next);
                player.addMediaItem(insertLocation, mediaItem);
                Log.d("applyDescriptions", "aadd " +  player.getMediaItemAt(0).mediaMetadata.title);
...
//        long messageId = currentMediaItem.mediaMetadata.extras.getLong(VoiceNoteMediaItemFactory.EXTRA_MESSAGE_ID);
        ...
        SimpleTask.run(EXECUTOR,
                () -> loadMediaItemsForConsecutivePlayback(0, latestUri),
                mediaItems -> {
    ...
    private @NonNull List<MediaItem> loadMediaItemsForSinglePlayback(long messageId, Uri uri) {
        Log.d("AAA", "loadMediaItemsForSinglePlayback " + uri);
        try {
...
//                .singletonList(VoiceNoteMediaItemFactory.buildMediaItem(context, threadId, draftUri));
    ...
    @WorkerThread
    private @NonNull List<MediaItem> loadMediaItemsForConsecutivePlayback(long messageId, Uri uri) {
        Log.d("AAA", "loadMediaItemsForConsecutivePlayback " + uri);
        try {
